...
def create_context_manager(
    ...
    # Model-specific context window limits (input + output tokens combined)
    model_limits = {
        "gpt-4.1": 2000000,  # 2M context window
        "gpt-4.1-mini": 2000000,  # 2M context window  
        "gpt-4.1-nano": 2000000,  # 2M context window
        "o3": 1000000,  # 1M context window
        "o4-mini": 2000000,  # 2M context window
        "gpt-4o": 128000,  # 128k context window
        "gpt-4o-mini": 128000,  # 128k context window
        "gpt-4o-realtime-preview": 128000,  # 128k context window
        "gpt-4": 8192,  # 8k context window
        "gpt-3.5-turbo": 16385,  # 16k context window
    }
    
    max_context = model_limits.get(model, 8192)  # Default to gpt-4 limit
    
    # For very large context models (>500k), use more conservative limits to avoid costs
    if max_context >= 500000:
        # Use only 10% of available context for conversation, reserve rest for response
        max_tokens = int(max_context * 0.1)
        summary_threshold = int(max_tokens * 0.8)
        min_recent = 10  # Keep more recent messages for large context models
    elif max_context >= 100000:
        # Use 50% of context for conversation for large models
        max_tokens = int(max_context * 0.5)
        summary_threshold = int(max_tokens * 0.8)
        min_recent = 8
    else:
        # Use 70% of context for smaller models
