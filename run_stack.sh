...
# ├───────────────┼────────────────────────┤
# │ Pane 6        │ Pane 7                 │
# │ ensemble_agent_client.py │ exchange_ui_service.py │
# └────────────────────────────────────────┘
###############################################################################
...
# 8. Pane 7 – exchange UI service (split Pane 6 horizontally ←)
tmux select-pane  -t $ENS_PANE
UI_PANE=$(tmux split-window -h -P -F "#{pane_id}")
tmux send-keys    -t $UI_PANE 'sleep 2 && source .venv/bin/activate && PYTHONPATH="$PWD" python agents/exchange_ui_service.py' C-m
